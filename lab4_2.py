...
"""Для дорог лучше подходит метод TRESH_TOZERO, т.к. картинка монотонна и полученное изображение четчё при ручном подборе трешхолда"""
...
img3 = cv2.adaptiveThreshold(img, maxValue = 255, adaptiveMethod = cv2.ADAPTIVE_THRESH_MEAN_C, 
                            treshholdType = cv2.THRESH_BINARY, blockSize = 11, C = -4)
